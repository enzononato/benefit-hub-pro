-- Função RPC para criar solicitações via bot n8n
...
BEGIN
  -- 1. Limpar o telefone (remover tudo que não é número)
  ...
  ELSIF length(v_clean_phone) = 12 AND v_clean_phone LIKE '55%' THEN
    v_clean_phone := substring(v_clean_phone from 3);
  END IF;
  
  -- 2. Buscar usuário pelo telefone na tabela profiles
  SELECT user_id INTO v_user_id
  FROM profiles
  WHERE regexp_replace(phone, '[^0-9]', '', 'g') = v_clean_phone
  LIMIT 1;
  
  -- Se não encontrou, tentar buscar com o telefone original limpo
  IF v_user_id IS NULL THEN
    SELECT user_id INTO v_user_id
    FROM profiles
    WHERE regexp_replace(phone, '[^0-9]', '', 'g') LIKE '%' || v_clean_phone || '%'
       OR v_clean_phone LIKE '%' || regexp_replace(phone, '[^0-9]', '', 'g') || '%'
    LIMIT 1;
  END IF;
  ...
  IF v_user_id IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'message', 'Usuário não encontrado com o telefone informado: ' || p_phone
    );
  END IF;
